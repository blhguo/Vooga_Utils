package voogasalad.util.voogleimages;
...
import voogasalad.util.voogleimages.view.NavigationBar;
import voogasalad.util.voogleimages.view.download.DownloadBar;
...
public class VoogleImages {
	...
	public static final String DEFAULT_RESOURCE_PACKAGE = "voogasalad/util/voogle_images/resources/";
	...
	private AttributionDialog myAttribution;
	...
	public VoogleImages(ImageObserver imageObserver) {
		...
		myAttribution = new AttributionDialog();
		makeFileChooser();
	...
	/**
	 * This is a simple class that manages ensuring the user understands the terms of attribution before
	 * using an image from the internet.
	 */
	private class AttributionDialog {

		private Alert myAlert;

		/**
		 * Creates the Attribution Dialog.
		 */
		private AttributionDialog() {
			ButtonType understand = new ButtonType(myResources.getString("Understand"), ButtonBar.ButtonData.OK_DONE);
			ButtonType cancel = new ButtonType(myResources.getString("Cancel"), ButtonBar.ButtonData.CANCEL_CLOSE);

			myAlert = new Alert(AlertType.WARNING, myResources.getString("Attribution"), cancel, understand);
			myAlert.setHeaderText(myResources.getString("AttributionTitle"));
		}

		/**
		 * Performs some extra logic checks to ensure that the user has understood the terms before
		 * advancing.
		 * 
		 * @return boolean specifying whether user chooses to advance or not
		 */
		private boolean showAndWait() {
			Optional<ButtonType> response = myAlert.showAndWait();
			return response.isPresent() && response.get().getButtonData().equals(ButtonBar.ButtonData.OK_DONE);
		}
